package com.xavier.flink.tutorial.chapter6;
...
public class MapStateExample {
    ...
    private static class MapStateFunction extends RichFlatMapFunction<UserBehavior, Tuple3<Long, String, Integer>> {
        ...
        public void open(Configuration parameters) throws Exception {
            // 创建 StateDescriptor
            MapStateDescriptor<String, Integer> behaviorMapStateDescriptor = new MapStateDescriptor<>(
                    "behaviorMap", Types.STRING, Types.INT);
        ...
        public void flatMap(UserBehavior value, Collector<Tuple3<Long, String, Integer>> out) throws Exception {
            int behaviorCnt = 1;
            // behavior 有可能为pv、cart、fav、buy等
            // 判断状态中是否有该behavior
            if (behaviorMapState.contains(value.behavior)) {
                behaviorCnt = behaviorMapState.get(value.behavior) + 1;
            }
            // 更新状态
            behaviorMapState.put(value.behavior, behaviorCnt);
            out.collect(Tuple3.of(value.userId, value.behavior, behaviorCnt));
        }
    }
}
